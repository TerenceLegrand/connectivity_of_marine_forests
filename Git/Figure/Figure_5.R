...
pd.range <- data.frame(x.min = c(max(pd.in.house),min(pd.third.party),min(pd.in.house)),
                       ...
                       y = c(0,0.5,1),
                       label = c("Long Distance Dispersal","Rafting","Propagule Dispersal"))
...
plot_boxplot <- ggplot(step_nbr, aes(PD, step)) + 
  ...
  scale_x_discrete(labels=c('LDD','Fixed')) +
...
layout <- c(
  patchwork::area(t = 0, l = 0, b = 3, r = 9),
  patchwork::area(t = 4, l = 0, b = 10, r = 9),
  patchwork::area(t = 4, l = 11, b = 10, r = 20),
  patchwork::area(t = 11, l = 0, b = 30, r = 20)
)

figure_5 <- plot_range + plot_density + plot_boxplot + plot_connexions_LDD +
  plot_layout(design = layout) + plot_annotation(tag_levels = 'A')

ggsave(filename = paste0("/Figure_5",".pdf"),
       plot = figure_5,
       device = cairo_pdf,
