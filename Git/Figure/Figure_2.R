...
correlation.final$Species_i <- paste(correlation.final$Species,correlation.final$count)
...
correlation.final$species_plot <- paste0(substr(correlation.final$genus,1,1),". ",sub(".*_", "", correlation.final$Species))
...
traits <- correlation.final %>%
  ggplot() +
  geom_text(aes(x = 0, y = Species_i, label = signif_SM_plot),  hjust = 0, size = 2,  colour = color[1]) +
  geom_text(aes(x = 0.01, y = Species_i, label = signif_CCM_plot),  hjust = 0, size = 2,  colour = color[3]) +
  geom_text(aes(x = 0.03, y = Species_i, label = data_id),  hjust = 0, size = 2) +
...
layout <- c(
  area(t = 0, l = 5, b = 4.5, r = 12),
  area(t = 0, l = 13, b = 8, r = 20),
  area(t = 5, l = 0, b = 30, r = 5), # left plot, starts at the top of the page (0) and goes 30 units down and 3 units to the right
  area(t = 5, l = 5, b = 30, r = 12),
  area(t = 5, l = 13, b = 30, r = 20)
)


# final plot arrangement
figure_2 <- boxplot + title + genus +forest_plot + traits +  plot_layout(design = layout)

ggsave(filename = paste0("/Figure_2",".pdf"),
       plot = figure_2,
       device = cairo_pdf,
